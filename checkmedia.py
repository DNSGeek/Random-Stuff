...
def openDB(dbfile = '/var/tmp/media.db'):
    try:
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS media (path TEXT NOT NULL, md5 TEXT NOT NULL, atime UNSIGNED INT NOT NULL, mtime UNSINGED INT NOT NULL, ctime UNSIGNED INT NOT NULL, size UNSIGNED INT NOT NULL, whent DATETIME default current_timestamp);")
    except Exception as ex:
...
def insertDB(cur, path, fhash, atime, mtime, ctime, size):
    tlock.acquire()
    try:
        cur.execute("INSERT INTO media VALUES(?, ?, ?, ?, ?, ?, current_timestamp);", (str(path), str(fhash), atime, mtime, ctime, size))
        tqueue.put(path)
    except Exception as ex:
        tlock.release()
...
def getDB(cur, path):
    tlock.acquire()
    try:
        cur.execute("SELECT md5, atime, mtime, ctime, size, whent FROM media WHERE path=? ORDER BY whent DESC LIMIT 1;", (str(path),))
    ...
    try:
        ...
        rtime = time.time()
    return((str(results[0]), int(results[1]), int(results[2]), int(results[3]), int(results[4]), rtime))
...
def hashThreads(cur, paths):
    for fullpath in paths:
        ...
        fhash = computeHash(fullpath)
        if fhash == "" or fhash == dbhash: continue
        insertDB(cur, fullpath, fhash, fstat[1], fstat[2], fstat[3], fstat[0])
