...
# Will this be a multi-threaded or multi-process server?
# Only have one of these uncommented. Choose what's best for your app.
Worker = Thread
#Worker = Process
...
def decode_quova(data):
    try:
        IP=data[-4:]
        octets = u"%d.%d.%d.%d" % (IP[0], IP[1], IP[2], IP[3])
        return(octets)
    except Exception as ex:
...
def encode_quova(ipinfo, IP):
    ...
    try:
        ...
        resba = bytearray(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x0c\x00\x00\x00\x00\x00\x00\x00\x06Mapped\xff\xff\xff\xff\x00\x00\x00\x0e\x00\x00\x00')
        sep = bytearray(b'\xff\xff\xff\xff\x00\x00\x00')
        resba.append(len(unicode(ipinfo['country_name'])))
        resba = uni_to_ba(resba, unicode(ipinfo['country_name']).lower())
        ...
        resba.append(len(unicode(ipinfo['city'])))
        resba = uni_to_ba(resba, unicode(ipinfo['city']).lower())
        resba += (sep + b'\x03\x00\x00\x00')
        resba.append(len(unicode(ipinfo['country_code'])))
        resba = uni_to_ba(resba, unicode(ipinfo['country_code']).lower())
        ...
        syslog.syslog("Error generating Quova packet for IP %s: %s. GeoIP info was %s" % (str(IP), str(ex), str(ipinfo)))
    ...
    while True: # The thread will go away when the connection closes.
        ...
        try:
            ...
            try:
                ...
                syslog.syslog("Unable to send GeoIP response for IP %s: %s" % (str(IP), str(ex)))
                return
    return
