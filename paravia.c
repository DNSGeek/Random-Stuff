...
void GenerateIncome(player *Me) {
  ...
  Me->CustomsDutyRevenue =
      Me->Nobles * 180 + Me->Clergy * 75 + Me->Merchants * 20 * (int)y;
  Me->CustomsDutyRevenue += (int)(Me->PublicWorks * 100.0f);
  Me->CustomsDutyRevenue =
      (int)((float)Me->CustomsDuty / 100.0f * (float)Me->CustomsDutyRevenue);

  Me->SalesTaxRevenue =
      Me->Nobles * 50 + Me->Merchants * 25 + (int)(Me->PublicWorks * 10.0f);
  Me->SalesTaxRevenue *= (int)(y * (5 - Me->Justice) * Me->SalesTax);
  Me->SalesTaxRevenue /= 200;

  Me->IncomeTaxRevenue = Me->Nobles * 250 + (int)(Me->PublicWorks * 20.0f);
  ...
  MsgPrint("Revenues: %d fl  Customs:%d Sales:%d Income:%d Justice:%d (%s)",
           ...
           Me->IncomeTaxRevenue, Me->JusticeRevenue, justStr);
}
...
void ChangeTitle(player *Me) {
  const char *t =
      Me->MaleOrFemale ? MaleTitles[Me->TitleNum] : FemaleTitles[Me->TitleNum];
...
void NewLandAndGrainPrices(player *Me) {
  ...
  Me->GrainDemand = Me->Nobles * 100 + Me->Cathedral * 40 + Me->Merchants * 30 +
                    Me->Soldiers * 10 + Me->Serfs * 5;

  Me->LandPrice =
      (3.0f * (float)Me->Harvest + (float)Random(6) + 10.0f) / 10.0f;
...
void PrintGrain(player *Me) {
  ...
  case 0:
  case 1:
    msg = "Drought. Famine threatens.";
    break;
  case 2:
    msg = "Bad weather. Poor harvest.";
    break;
  case 3:
    msg = "Normal weather. Average harvest.";
    break;
  case 4:
    msg = "Good weather. Fine harvest.";
    break;
  default:
    msg = "Excellent weather. Great harvest!";
    break;
  }
...
int ReleaseGrain(player *Me) {
  ...
  if (HowMuch < (Me->GrainDemand - 1)) {
    x = ((float)Me->GrainDemand - (float)HowMuch) / (float)Me->GrainDemand *
            100.0f -
        9.0f;
    xp = (double)x;
    ...
    if (HowMuch > (int)((float)Me->GrainDemand * 1.3f)) {
      ...
      z = ((float)HowMuch - (float)Me->GrainDemand) / (float)Me->GrainDemand *
          10.0f;
  ...
  MsgPrint("Paid soldiers %d florins.  Serfs in city: %d.", Me->SoldierPay,
           Me->Serfs);
...
void SeizeAssets(player *Me) {
  ...
  MsgPrint("%s %s is BANKRUPT. Creditors seize your assets.", Me->Title,
           Me->Name);
...
void SerfsDecomposing(player *Me, float scale) {
  ...
  Me->DeadSerfs =
      (int)(((float)Random(absc) + ord) * (float)Me->Serfs / 100.0f);
...
void SerfsProcreating(player *Me, float scale) {
  int absc = (int)scale;
  float ord = scale - (float)absc;
  Me->NewSerfs = (int)(((float)Random(absc) + ord) * (float)Me->Serfs / 100.0f);
...
void PlayGame(player players[], int n) {
  ...
  if (allDead) {
    ...
    beep();
    MsgPrint("GAME OVER.  %s %s WINS!", players[winningPlayer].Title,
...
void NewTurn(player *Me, int howMany, player players[], player *Baron) {
  ...
  if (Me->InvadeMe) {
    for (i = 0; i < howMany; i++) {
      if (i != Me->WhichPlayer && players[i].Soldiers > (Me->Soldiers * 2.4f)) {
...
void BuySellGrain(player *Me) {
  ...
  while (true) {
    MsgClear();
    MsgPrint("Year %d — %s %s   Rats ate %d%% of grain.", Me->Year, Me->Title,
             Me->Name, Me->Rats);
    PrintGrain(Me);
    DrawMap(Me);
    DrawStats(Me);
...
void AdjustTax(player *Me) {
  ...
  while (true) {
    ...
    GenerateIncome(Me);
    DrawStats(Me);
    MsgPrint("Customs:%d%%  Sales:%d%%  Income:%d%%", Me->CustomsDuty,
             Me->SalesTax, Me->IncomeTax);
    ...
    case 4:
      duty = MsgGetInt(
          "Justice 1=Very Fair 2=Moderate 3=Harsh 4=Outrageous: ", 1, 4);
      Me->Justice = duty;
      break;
    }
    DrawStats(Me);
...
void ShowStats(player players[], int howMany) {
  MsgClear();
  MsgPrint("%-12s %6s %8s %6s %9s %6s %7s %8s", "Name", "Nobles", "Soldiers",
           "Clergy", "Merchants", "Serfs", "Land", "Treasury");

  for (int i = 0; i < howMany; i++) {
    MsgPrint("%-12s %6d %8d %6d %9d %6d %7d %8d", players[i].Name,
             players[i].Nobles, players[i].Soldiers, players[i].Clergy,
             players[i].Merchants, players[i].Serfs, players[i].Land,
             players[i].Treasury);
  }
  MsgWaitEnter();
}
